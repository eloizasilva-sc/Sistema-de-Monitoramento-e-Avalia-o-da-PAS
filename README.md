...
.login-card {
  ...
  box-shadow: 0 20px 60px rgba(2,6,23,0.45), 0 6px 18px rgba(37,99,235,0.15);
}
...
.login-card .tab-btn.text-slate-400 { color: #64748b !important; }
      ...
      metasOrdenadas.forEach(meta => {
        ...
        const areasTexto = (meta.areas && Array.isArray(meta.areas)) ? meta.areas.join(' • ') : '';

        // Verificar se tem monitoramento neste quadrimestre (baseado nos progressos das ações)
        const temMonitoramentoQuad = acoesVinculadas.some(a => 
          a.progressos && a.progressos.some(p => p.quadrimestre === quadrimestre)
        );

        // Buscar monitoramento da própria meta, se existir
          ...
          <div class="bg-white rounded-xl border-2 border-blue-200 p-6 mb-4 shadow-sm hover:shadow-md transition">
              ...
              <div class="flex-1">
                <div class="flex items-center gap-3 mb-2">
                  ...
                  <div class="flex-1">
                    ...
                    ${monitoramentoMeta ? `<p class="text-sm text-green-700 font-semibold mt-2">Resultado: ${monitoramentoMeta.resultado} • Status: ${monitoramentoMeta.status}</p>` : ''}
                  </div>
                </div>
              </div>
            ...
            <div class="bg-blue-50 rounded-lg p-4 mb-4 border border-blue-200">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <p class="text-xs font-bold text-blue-700 mb-1">📊 INDICADOR</p>
                ...
                <div>
                  <p class="text-xs font-bold text-blue-700 mb-1">✅ AÇÕES VINCULADAS</p>
                  <p class="text-sm font-semibold text-blue-900">${acoesVinculadas.length} ação(ões)</p>
                </div>
              </div>
            </div>
            ...
            ${temMonitoramentoQuad ? `
              <div class="bg-green-50 rounded-lg p-4 border border-green-200 mb-4">
                <p class="text-xs font-bold text-green-700 mb-2">✅ Monitoramento preenchido para este quadrimestre</p>
              </div>
            ` : `
              <div class="bg-yellow-50 rounded-lg p-4 border border-yellow-200 mb-4">
                <p class="text-sm text-yellow-800 font-semibold">⚠️ Monitoramento pendente para este quadrimestre</p>
              </div>
            `}

            <!-- LISTA DE AÇÕES VINCULADAS (EXIBIDAS ABAIXO DA META) -->
                ...
                return `
                  <div class="border-2 border-slate-200 rounded-lg p-3 bg-white">
                    ...
                    <div class="text-xs text-slate-600 mb-2">
                      ...
                      <div><strong>Valor Executado:</strong> ${formatToBRL(progressoExistente?.valorExecutado) || (progressoExistente?.valorExecutado || '—')}</div>
                    </div>
                    <div>
                      ...
                      <div style="font-size:11px;margin-top:6px;color:#374151;font-weight:700;">${pct}% • ${progressoExistente?.status || 'Sem preenchimento'}</div>
                    </div>
 ...
 <body class="h-full bg-gradient-to-br from-blue-700 to-blue-900 text-white"><!-- TELA DE LOGIN -->
  ...
  <div class="relative z-10 login-card rounded-3xl p-8 w-full max-w-md mx-4 animate-slide-up">
    <div class="text-center mb-8">
     ...
     <p class="text-sm text-slate-600 font-semibold leading-relaxed">Sistema de Monitoramento e Avaliação<br>
      da Programação Anual de Saúde</p>
    </div>
    ...
    <form id="loginForm">
     ...
     <div class="mb-5"><label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wide">E-mail ou Usuário</label>
    ...
    <div class="mt-8 pt-6 border-t border-slate-200 text-center">
     ...
     <p class="text-xs text-slate-500 italic">Especialização em Saúde Coletiva com Área de Concentração em Monitoramento e Avaliação em Saúde</p>
    </div>
   </div>
  ...
  <div id="mainSystem" style="display: none;" class="h-full flex">
    ...
    <nav class="flex-1 p-4 overflow-y-auto">
     <div class="space-y-1"><a href="#" onclick="mostrarSecao('dashboard')" class="nav-item active flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition group" data-secao="dashboard"> <i data-lucide="layout-dashboard" class="w-5 h-5"></i> <span class="font-semibold text-sm">Painel Geral</span> </a> <a href="#" onclick="mostrarSecao('cadastro')" class="nav-item flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition group" data-secao="cadastro" id="navCadastro"> <i data-lucide="file-plus" class="w-5 h-5"></i> <span class="font-semibold text-sm">Cadastro PAS</span> </a> <a href="#" onclick="mostrarSecao('monitoramento')" class="nav-item flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition group" data-secao="monitoramento"> <i data-lucide="trending-up" class="w-5 h-5"></i> <span class="font-semibold text-sm">Monitoramento</span> </a>
      <div class="pt-4" id="navAdminContainer" style="display: none;">
       <div class="pb-2">
        <p class="px-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Administração</p>
       </div><a href="#" onclick="mostrarSecao('admin')" class="nav-item flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition group" data-secao="admin"> <i data-lucide="settings" class="w-5 h-5 text-slate-400"></i> <span class="font-semibold text-sm">Configurações</span> </a>
      </div>
     </div>
    </nav>
    <div class="p-4 border-t border-slate-800">
     <div class="flex items-center gap-3 p-3 bg-slate-800 rounded-xl">
      ...
      <div class="flex-1 min-w-0">
       <p id="userName" class="font-semibold text-sm truncate">Usuário</p>
   ...
   <main class="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto">
     <div class="p-6 max-w-[1600px] mx-auto pb-32"><!-- SEÇÃO DASHBOARD -->
      <div id="secaoDashboard" class="secao-conteudo">
       <div class="bg-white rounded-xl border border-slate-200 p-6 mb-6 shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-4">
         <h2 class="text-lg font-bold text-slate-900 flex items-center gap-2"><i data-lucide="filter" class="w-5 h-5 text-blue-600"></i> Filtrar por Área</h2>
       ...
       <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <div class="bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl p-6 text-white shadow-lg">
         ...
         <p class="text-sm font-semibold opacity-90">Total de Ações</p>
         <p class="text-xs opacity-75 mt-1">Programação Anual</p>
        </div>
        <div class="bg-gradient-to-br from-red-600 to-red-700 rounded-xl p-6 text-white shadow-lg">
         ...
         <p class="text-sm font-semibold opacity-90">Crítico</p>
         <p class="text-xs opacity-75 mt-1">Ações Pendentes</p>
        </div>
        <div class="bg-gradient-to-br from-green-600 to-green-700 rounded-xl p-6 text-white shadow-lg">
         ...
         <p class="text-xs opacity-75 mt-1" id="semaforoPanorama">🔴 Crítico</p>
        </div>
       ...
       <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
         <h3 class="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2"><i data-lucide="pie-chart" class="w-5 h-5 text-blue-600"></i> Distribuição por Status</h3>
       ...
       <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        <h3 class="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2"><i data-lucide="building-2" class="w-5 h-5 text-blue-600"></i> Distribuição por Área Técnica</h3>
      ...
      <div id="secaoMonitoramento" class="secao-conteudo" style="display: none;">
       <div class="bg-white rounded-xl border border-slate-200 p-6 mb-6 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
         <div><label class="block text-sm font-bold text-slate-700 mb-2">Filtrar por Área</label>
        ...
        <div id="areaDefinirPrazos" class="mt-6 pt-6 border-t-2 border-slate-200" style="display: none;">
         <h3 class="text-md font-bold text-slate-900 mb-3 flex items-center gap-2"><i data-lucide="calendar-clock" class="w-5 h-5 text-purple-600"></i> Gestão de Prazos e Liberação dos Quadrimestres</h3>
         <p class="text-sm text-slate-600 mb-4">Defina os prazos de preenchimento e libere o acesso aos quadrimestres para técnicos e visualizadores</p>
         <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div class="${prazo ? status.classe + ' border-2' : 'bg-slate-100 border-2 border-dashed border-slate-300'} rounded-lg p-4 transition">
           <div class="flex items-center justify-between mb-2"><span class="font-bold text-sm text-slate-900">${quad}</span>
            <div class="flex gap-1"><button onclick="definirPrazoQuadrimestre('${quad}')" class="p-1 hover:bg-black/10 rounded transition" title="Gerenciar prazo e liberação"> <i data-lucide="${prazo ? 'edit-2' : 'calendar-plus'}" class="w-4 h-4 text-slate-900"></i> </button>
            </div>
           </div> ${prazo ? ` 
           <p class="text-xs font-semibold mb-1">${status.emoji} ${status.texto}</p>
           <p class="text-xs opacity-75 mb-2">📅 ${new Date(prazo).toLocaleDateString('pt-BR')}</p> ` : ` 
           <p class="text-xs text-slate-600 mb-2">Clique para definir prazo</p> `} 
           <div class="flex items-center gap-2 pt-2 border-t ${prazo ? 'border-slate-300/50' : 'border-slate-300'}"><span class="text-xs font-bold ${liberado ? 'text-green-700' : 'text-red-700'}">${liberado ? '🟢 Liberado' : '🔴 Bloqueado'}</span>
           </div>
          </div> `; }).join('')}
         </div>
        </div>
      ...
      </div><!-- SEÇÃO ADMINISTRAÇÃO -->
      <div id="secaoAdmin" class="secao-conteudo" style="display: none;">
       <div id="alertaUsuariosPendentes" class="mb-6"></div>
       <div class="bg-white rounded-xl border border-slate-200 p-2 mb-6 shadow-sm flex gap-2 flex-wrap"><button onclick="mostrarAbaAdmin('autorizacao')" class="aba-admin-btn active flex-1 min-w-[150px] px-4 py-3 rounded-lg font-bold text-sm transition-all" data-aba="autorizacao"> <i data-lucide="user-check" class="w-4 h-4 inline mr-2"></i> Autorização de Usuários </button> <button onclick="mostrarAbaAdmin('usuarios')" class="aba-admin-btn flex-1 min-w-[150px] px-4 py-3 rounded-lg font-bold text-sm transition-all" data-aba="usuarios"> <i data-lucide="users" class="w-4 h-4 inline mr-2"></i> Gestão de Usuários </button> <button onclick="mostrarAbaAdmin('auditoria')" class="aba-admin-btn flex-1 min-w-[150px] px-4 py-3 rounded-lg font-bold text-sm transition-all" data-aba="auditoria"> <i data-lucide="shield" class="w-4 h-4 inline mr-2"></i> Auditoria do Sistema </button> <button onclick="mostrarAbaAdmin('configuracoes')" class="aba-admin-btn flex-1 min-w-[150px] px-4 py-3 rounded-lg font-bold text-sm transition-all" data-aba="configuracoes"> <i data-lucide="settings" class="w-4 h-4 inline mr-2"></i> Configurações </button>
       </div>
       <div id="abaAutorizacao" class="aba-admin-conteudo">
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
         <h3 class="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2"><i data-lucide="user-check" class="w-5 h-5 text-blue-600"></i> Solicitações Pendentes de Autorização</h3>
         <div id="listaAutorizacoes"></div>
        </div>
       </div>
       <div id="abaUsuarios" class="aba-admin-conteudo" style="display: none;">
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
         <h3 class="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2"><i data-lucide="users" class="w-5 h-5 text-blue-600"></i> Todos os Usuários do Sistema</h3>
       ...
       <div id="abaConfiguracoes" class="aba-admin-conteudo" style="display: none;">
        <div class="bg-gradient-to-br from-blue-600 to-blue-800 rounded-xl p-6 text-white mb-6 shadow-lg">
         <h3 class="text-2xl font-black mb-2 flex items-center gap-2"><i data-lucide="bar-chart-3" class="w-7 h-7"></i> Métricas do Sistema</h3>
         <p class="text-blue-100 text-sm">Tempo médio de preenchimento e estatísticas gerais</p>
        </div>
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
         <div class="bg-white rounded-xl border-2 border-slate-200 p-6 shadow-sm">
          <h4 class="text-sm font-bold text-slate-700 mb-4 uppercase tracking-wide flex items-center gap-2"><i data-lucide="clock" class="w-4 h-4 text-blue-600"></i> Tempo Médio de Preenchimento</h4>
         ...
         <div class="bg-white rounded-xl border-2 border-slate-200 p-6 shadow-sm">
          <h4 class="text-sm font-bold text-slate-700 mb-4 uppercase tracking-wide flex items-center gap-2"><i data-lucide="activity" class="w-4 h-4 text-purple-600"></i> Estatísticas Gerais</h4>
          <div id="metricasGerais" class="space-y-3"></div>
         </div>
        </div>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
         <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h3 class="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2"><i data-lucide="building-2" class="w-5 h-5 text-blue-600"></i> Gestão de Áreas Técnicas</h3>
          <form id="formNovaArea" class="flex gap-3 mb-6"><input type="text" id="novaAreaNome" class="flex-1 px-4 py-2.5 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition text-slate-900" placeholder="Nome da nova área (ex: SAPS)" required> <button type="submit" class="px-6 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-bold text-sm whitespace-nowrap"> Adicionar </button>
          </form>
          <div id="listaAreas" class="space-y-2"></div>
         </div>
         <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h3 class="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2"><i data-lucide="calendar" class="w-5 h-5 text-blue-600"></i> Gestão de Quadrimestres</h3>
          <form id="formNovoQuadrimestre" class="flex gap-3 mb-6"><input type="text" id="novoQuadrimestreNome" class="flex-1 px-4 py-2.5 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition text-slate-900" placeholder="Nome do quadrimestre (ex: 1º Quadrimestre)" required> <button type="submit" class="px-6 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-bold text-sm whitespace-nowrap"> Adicionar </button>
          </form>
     ...
     <footer class="fixed bottom-0 left-0 lg:left-72 right-0 bg-gradient-to-r from-blue-600 to-blue-800 text-white py-4 px-6 shadow-lg z-30">
      <div class="max-w-[1600px] mx-auto text-center">
       ...
       <p class="text-xs text-blue-100 italic">Especialização em Sade Coletiva com Área de Concentração em Monitoramento e Avaliação em Saúde</p>
      </div>
     </footer>
    </div>
   </main>
  ...
  <div id="modalNovaMeta" class="modal-overlay" style="display: none;">
   <div class="modal-container w-full max-w-2xl p-8">
    ...
    <form id="formNovaMeta">
     <div class="space-y-4">
      <div><label class="block text-sm font-bold text-slate-700 mb-2">Título da Meta *</label> <input type="text" id="metaTitulo" class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition text-slate-900" placeholder="Ex: Ampliar cobertura vacinal" required>
      </div>
      <div><label class="block text-sm font-bold text-slate-700 mb-2">Objetivo Estratégico *</label> <textarea id="metaObjetivo" rows="3" class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition resize-none text-slate-900" placeholder="Descreva o objetivo estratégico da meta" required></textarea>
      </div>
      ...
      <div><label class="block text-sm font-bold text-slate-700 mb-2">Áreas Técnicas *</label>
       <div id="metaAreasCheckboxes" class="border-2 border-slate-200 rounded-lg p-4 space-y-2 max-h-48 overflow-y-auto"></div>
       <p class="text-xs text-slate-500 mt-1">Selecione uma ou mais áreas responsáveis</p>
      </div>
  ...
  <div id="modalNovaAcao" class="modal-overlay" style="display: none;">
   <div class="modal-container w-full max-w-3xl p-8">
    <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-200">
     <h2 class="text-2xl font-bold text-slate-900 flex items-center gap-3"><i data-lucide="plus-circle" class="w-6 h-6 text-green-600"></i> <span id="tituloModalAcao">Nova Ação</span></h2><button onclick="fecharModalNovaAcao()" class="p-2 hover:bg-slate-100 rounded-lg transition"> <i data-lucide="x" class="w-6 h-6 text-slate-400"></i> </button>
    </div>
    <form id="formNovaAcao">
     <div id="infoMetaVinculada" class="mb-4 p-4 bg-blue-50 border-2 border-blue-200 rounded-lg" style="display: none;">
      <div class="flex items-center gap-2 text-blue-800"><i data-lucide="link" class="w-5 h-5"></i> <span class="font-bold text-sm">Vinculada à meta:</span> <span id="metaVinculadaNome" class="font-semibold"></span>
      </div>
     </div>
     <div class="space-y-4">
      <div><label class="block text-sm font-bold text-slate-700 mb-2">Código da Ação *</label> <input type="text" id="acaoTitulo" class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition text-slate-900" placeholder="Ex: APS-001" required>
      </div>
      <div><label class="block text-sm font-bold text-slate-700 mb-2">Descrição da Ação *</label> <textarea id="acaoDescricao" rows="3" class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition resize-none text-slate-900" placeholder="Descreva detalhadamente a ação a ser realizada" required></textarea>
      </div>
  ...
  <div id="modalMonitoramento" class="modal-overlay" style="display: none;">
   <div class="modal-container w-full max-w-6xl p-8 max-h-[95vh] overflow-y-auto">
    ...
    <form id="formMonitoramento">
     <div class="space-y-6"><!-- SE��ÃO MONITORAMENTO DA META -->
      <div id="secaoMonitoramentoMeta" class="bg-gradient-to-br from-blue-50 to-blue-100 border-2 border-blue-400 rounded-xl p-6 shadow-lg">
       <h3 class="text-xl font-black text-blue-900 mb-4 flex items-center gap-3"><i data-lucide="target" class="w-6 h-6"></i> 📊 Monitoramento da Meta</h3><!-- Indicadores Fixos da Meta -->
       <div class="bg-white rounded-lg p-5 border-2 border-blue-300 mb-5 shadow-md">
        <p class="text-sm font-black text-blue-900 mb-4 uppercase tracking-wide flex items-center gap-2"><i data-lucide="lock" class="w-4 h-4"></i> Dados Fixos (Não Editáveis)</p>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
         <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
          <p class="text-xs font-bold text-blue-700 mb-2">📊 INDICADOR DE BASE</p>
          <p id="metaIndicadorFixo" class="text-2xl font-black text-blue-900">-</p>
         </div>
         <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
          <p class="text-xs font-bold text-blue-700 mb-2">🎯 % PROPOSTO</p>
          <p id="metaPercentualProposto" class="text-2xl font-black text-blue-900">-</p>
         </div>
        </div>
       </div>
       <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div><label class="block text-sm font-bold text-slate-700 mb-2">Resultado Alcançado *</label> <input type="text" id="monitorMetaResultado" class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition text-slate-900" placeholder="Ex: 85%" required>
         <p class="text-xs text-slate-500 mt-1">Resultado em percentual ou valor numérico</p>
        </div>
        <div><label class="block text-sm font-bold text-slate-700 mb-2">Status *</label> <select id="monitorMetaStatus" class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition text-slate-900" required> <option value="">Selecione...</option> <option value="CONCLUIDA">✅ Concluída</option> <option value="EM_ANDAMENTO">⏳ Em Andamento</option> <option value="PREPARATORIA"> Preparatória</option> <option value="NAO_INICIADA"> Não Iniciada</option> <option value="PARALISADA">⏸️ Paralisada</option> </select>
        </div>
       </div>
       <div class="mt-4"><label class="block text-sm font-bold text-slate-700 mb-2">Análise Crítica da Meta *</label> <textarea id="monitorMetaAnalise" rows="4" class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition resize-none text-slate-900" placeholder="Descreva a análise crítica da meta, resultados obtidos, desafios enfrentados..." required></textarea>
       </div>
      </div><!-- SEÇÃO MONITORAMENTO DAS AÇÕES VINCULADAS -->
      <div id="secaoMonitoramentoAcoes" class="bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-400 rounded-xl p-6 shadow-lg">
       <h3 class="text-xl font-black text-green-900 mb-4 flex items-center gap-3"><i data-lucide="list-checks" class="w-6 h-6"></i> ✅ Monitoramento das Ações Vinculadas</h3>
       <div id="listaAcoesMonitoramento" class="space-y-5"><!-- Será preenchido dinamicamente -->
       </div>
      </div>
  ...
  <script>
// C���DIGO JAVASCRIPT COMPLETO AQUI (continuação no pr��ximo bloco devido ao tamanho)
// [O código JavaScript permanece EXATAMENTE como estava na versão 77]
</script>
  <script>
...
let quadrimestres = JSON.parse(localStorage.getItem('quadrimestres')) || ['1º Quadrimestre', '2º Quadrimestre', '3º Quadrimestre'];
...
// ---------- UTILITÁRIOS: formatação BRL e parser ----------
...
document.getElementById('loginForm').addEventListener('submit', async (e) => {
  ...
  if (usuario.nivel !== 'ADMIN' && !usuario.autorizado) {
    showAlert('Sua conta está aguardando autorização do administrador', 'error', 'loginAlertContainer');
    btnTexto.textContent = 'Acessar Sistema';
    return;
  }
  
  currentUser = usuario;
  registrarAuditoria('LOGIN', `Usuário ${usuario.nome} acessou o sistema`);
  ...
  document.getElementById('userRole').textContent = perfilTexto[usuario.nivel] + (usuario.area ? ` • ${usuario.area}` : '');
  ...
  if (usuario.nivel === 'ADMIN') {
    ...
    // Técnicos não devem acessar o módulo de Cadastro PAS
...
document.getElementById('registroForm').addEventListener('submit', async (e) => {
  ...
  if (usuarios.some(u => u.email === email)) {
    showAlert('E-mail já cadastrado', 'error', 'registroAlertContainer');
    return;
  }
  ...
  showAlert('Conta criada! Aguarde a autorização do administrador para acessar.', 'success', 'registroAlertContainer');
...
function logout() {
  if (currentUser) {
    registrarAuditoria('LOGOUT', `Usuário ${currentUser.nome} saiu do sistema`);
  }
...
function renderizarFiltrosArea(containerId) {
  const container = document.getElementById(containerId);
  if (!container) return;
  // Mostrar todas as áreas no Painel Geral para todos os perfis
  ...
  let html = '<button class="px-4 py-2 rounded-lg font-bold text-sm transition ' + 
    ...
    '" onclick="aplicarFiltroArea(\'TODAS\')">Todas as Áreas</button>';
...
function calcularPanorama() {
  ...
  let semaforo = '🔴 Crítico';
  if (mediaProgresso >= 75) semaforo = '🟢 Excelente';
  else if (mediaProgresso >= 50) semaforo = '🟡 Atenção';
...
function renderizarGraficoStatus() {
  ...
  const stats = {
    'CONCLUIDA': { count: 0, label: 'Concluídas', color: 'bg-green-500', icon: 'check-circle' },
    'EM_ANDAMENTO': { count: 0, label: 'Em Andamento', color: 'bg-blue-500', icon: 'clock' },
    'PREPARATORIA': { count: 0, label: 'Preparatórias', color: 'bg-yellow-500', icon: 'settings' },
    'NAO_INICIADA': { count: 0, label: 'Não Iniciadas', color: 'bg-red-500', icon: 'alert-circle' },
    ...
    html += `
      <div class="space-y-2">
        <div class="flex items-center justify-between">
          ...
          <span class="text-sm font-bold text-slate-900">${count} ações</span>
        </div>
        <div class="w-full bg-slate-200 rounded-full h-3">
  ...
  areasVisiveis.forEach(area => {
    ...
    html += `
      <div class="bg-white border-2 border-slate-200 rounded-xl p-5 hover:shadow-lg transition">
        ...
        <p class="text-xs text-slate-600">${total} ações • ${concluidas} concluídas</p>
      </div>
    `;
  });
...
function renderizarMetasAcoes() {
  ...
  if (metasVisiveis.length === 0) {
    container.innerHTML = `
      <div class="bg-slate-100 rounded-xl p-12 text-center border-2 border-dashed border-slate-300 animate-fade-in">
        ...
        <p class="text-sm text-slate-600">Clique em "Nova Meta" para começar</p>
      </div>
    `;
  ...
  metasVisiveis.forEach(meta => {
    const acoesVinculadas = acoes.filter(a => a.metaId === meta.id);
    const areasTexto = Array.isArray(meta.areas) ? meta.areas.join(' • ') : '';
          ...
          <div class="flex-1">
            ...
            <div class="flex flex-wrap gap-3">
              <div class="bg-blue-50 px-3 py-1.5 rounded-lg">
                <span class="text-xs font-bold text-blue-900">📊 ${meta.indicador}</span>
              </div>
              <div class="bg-green-50 px-3 py-1.5 rounded-lg">
                <span class="text-xs font-bold text-green-900">🎯 ${meta.percentualProposto}%</span>
              </div>
            </div>
          </div>
          ${podeEditar ? `
        ...
        <div class="border-t-2 border-slate-200 pt-4">
          <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
            <h4 class="text-sm font-bold text-slate-700 flex items-center gap-2">
              ...
              Ações Vinculadas (${acoesVinculadas.length})
            </h4>
            ${podeEditar ? `
          ...
          ${acoesVinculadas.length === 0 ? `
            <p class="text-sm text-slate-500 italic">Nenhuma ação vinculada a esta meta</p>
          ` : `
            <div class="space-y-2">
              ${acoesVinculadas.map(acao => {
                ...
                return `
                  <div class="bg-slate-50 border border-slate-200 rounded-lg p-4">
                    <div class="flex items-start justify-between gap-4">
                      ...
                      ${podeEditarAcao ? `
                      <div class="flex gap-2 flex-shrink-0">
                        <button onclick="editarAcao('${acao.id}')" class="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition" title="Editar Ação">
                          <i data-lucide="edit-2" class="w-4 h-4"></i>
                        </button>
                        <button onclick="excluirAcao('${acao.id}')" class="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition" title="Excluir Ação">
                          <i data-lucide="trash-2" class="w-4 h-4"></i>
                        </button>
                      </div>
                      ` : ''}
...
function renderizarFiltrosAreaCadastro() {
  ...
  let html = '<button class="px-4 py-2 rounded-lg font-bold text-sm transition ' + 
    ...
    (currentUser && currentUser.nivel === 'TECNICO' ? 'Minha Área' : 'Todas as Áreas') + 
    '</button>';
...
function getStatusBadge(status) {
  const badges = {
    'CONCLUIDA': '<span class="status-badge bg-green-100 text-green-800">✅ Concluída</span>',
    'EM_ANDAMENTO': '<span class="status-badge bg-blue-100 text-blue-800">⏳ Em Andamento</span>',
    'PREPARATORIA': '<span class="status-badge bg-yellow-100 text-yellow-800">🔧 Preparatória</span>',
    'NAO_INICIADA': '<span class="status-badge bg-red-100 text-red-800">⚪ Não Iniciada</span>',
    'PARALISADA': '<span class="status-badge bg-gray-100 text-gray-800">⏸️ Paralisada</span>'
  };
...
document.getElementById('formNovaMeta').addEventListener('submit', (e) => {
  ...
  console.log('Formulário de meta submetido');
  ...
  if (areasSelecionadas.length === 0) {
    alert('⚠️ Selecione pelo menos uma área técnica');
    return;
  }
  ...
  salvarDados();
  console.log('Metas após salvar:', metas);
...
function excluirMeta(id) {
  ...
  confirmar.innerHTML = `
    <div class="modal-container w-full max-w-md p-6">
      <h3 class="text-xl font-bold text-slate-900 mb-4">Confirmar Exclusão</h3>
      <p class="text-sm text-slate-700 mb-4">Tem certeza que deseja excluir a meta "<strong>${meta.titulo}</strong>"?</p>
      ${acoesVinculadas.length > 0 ? `<p class="text-sm text-red-600 font-semibold mb-4">��️ Esta meta possui ${acoesVinculadas.length} ação(ões) vinculada(s) que também serão removidas!</p>` : ''}
...
function confirmarExclusaoMeta(id) {
  ...
  if (meta) {
    ...
    registrarAuditoria('EXCLUIR_META', `Meta "${meta.titulo}" excluída`);
    salvarDados();
...
document.getElementById('formNovaAcao').addEventListener('submit', (e) => {
  ...
  console.log('Formulário de ação submetido');
  ...
  console.log('Dados da ação:', { titulo, descricao, indicador, unidadeMedida, valorProposto, resultadoProposto, responsavel, areasSelecionadas, metaIdParaVincular });
  ...
  if (editId) {
    ...
    if (acao) {
      ...
      console.log('Ação editada:', acao);
      registrarAuditoria('EDITAR_ACAO', `Ação "${titulo}" editada`);
    }
  } else {
    const novaAcao = {
      id: Date.now().toString(),
    ...
    console.log('Nova ação criada:', novaAcao);
    acoes.push(novaAcao);
    
    registrarAuditoria('CRIAR_ACAO', `Nova ação "${titulo}" criada`);
  }
  
  salvarDados();
  console.log('Ações após salvar:', acoes);
...
function excluirAcao(id) {
  ...
  confirmar.innerHTML = `
    <div class="modal-container w-full max-w-md p-6">
      <h3 class="text-xl font-bold text-slate-900 mb-4">Confirmar Exclusão</h3>
      <p class="text-sm text-slate-700 mb-4">Tem certeza que deseja excluir a ação "<strong>${acao.titulo}</strong>"?</p>
      <div class="flex gap-3">
        <button onclick="this.closest('.modal-overlay').remove()" class="flex-1 px-4 py-2 border-2 border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 transition font-bold">
          Cancelar
        </button>
...
function confirmarExclusaoAcao(id) {
  ...
  if (acao) {
    ...
    registrarAuditoria('EXCLUIR_ACAO', `Ação "${acao.titulo}" excluída`);
    salvarDados();
...
function renderizarMonitoramento() {
  ...
  // Mostrar área de definir prazos apenas para admin
  ...
  if (areaPrazos && currentUser && currentUser.nivel === 'ADMIN') {
    ...
    // Re-renderizar os botões de prazo com status atualizado
      ...
      container.innerHTML = quadrimestres.map(quad => {
        ...
        return `
          <button onclick="definirPrazoQuadrimestre('${quad}')" class="${prazo ? status.classe + ' border-2' : 'bg-slate-100 border-2 border-dashed border-slate-300 hover:bg-slate-200'} rounded-lg p-4 text-left transition">
            ...
            ${prazo ? `
              <p class="text-xs font-semibold mb-1">${status.emoji} ${status.texto}</p>
              <p class="text-xs opacity-75">�� ${new Date(prazo).toLocaleDateString('pt-BR')}</p>
            ` : `
...
function renderizarFiltrosMonitoramento() {
  ...
  let htmlArea = '<button class="px-4 py-2 rounded-lg font-bold text-sm transition ' + 
    ...
    (currentUser && currentUser.nivel === 'TECNICO' ? 'Minha Área' : 'Todas') + 
    '</button>';
  ...
  // Filtrar quadrimestres para mostrar apenas liberados (se não for admin)
  ...
  if (quadrimestresVisiveis.length > 0) {
    ...
    quadrimestresVisiveis.forEach(quad => {
      ...
      htmlQuad += `<button class="px-4 py-2 rounded-lg font-bold text-sm transition ${
        ...
        ${quad}
        ${currentUser && currentUser.nivel === 'ADMIN' ? (liberado ? ' 🟢' : ' 🔴') : ''}
      </button>`;
    });
  } else if (currentUser && currentUser.nivel !== 'ADMIN') {
    htmlQuad = '<p class="text-sm text-yellow-700 font-semibold">🔒 Nenhum quadrimestre liberado para visualização</p>';
  }
...
function calcularStatusPrazo(dataLimite) {
  ...
  if (diffDays < 0) {
    ...
    return { 
      ...
      emoji: '🟡', 
      texto: `Atenção - ${diffDays} dia(s) restante(s)`,
      dias: diffDays
    };
  } else {
    return { 
...
function definirPrazoQuadrimestre(quadrimestre) {
      ...
      <form id="formDefinirPrazo" class="space-y-5">
        ...
        <div class="bg-green-50 rounded-lg p-4 border-2 border-green-200">
          ...
          <label class="flex items-center gap-3 cursor-pointer">
            ...
            <div class="flex-1">
              <p class="text-sm font-bold text-slate-900">Liberar visualização</p>
              <p class="text-xs text-slate-600">Permite que técnicos e visualizadores vejam as ações deste quadrimestre</p>
            </div>
          </label>
        </div>
  ...
  document.getElementById('formDefinirPrazo').addEventListener('submit', (e) => {
    ...
    registrarAuditoria('LIBERACAO_QUADRIMESTRE', `${quadrimestre} ${liberado ? 'liberado' : 'bloqueado'} para visualização`);
...
function removerPrazoQuadrimestre(quadrimestre) {
  ...
  registrarAuditoria('REMOVER_PRAZO', `Prazo e liberação do ${quadrimestre} removidos`);
  salvarDados();
...
function renderizarListaMonitoramento() {
          ...
          <div class="grid grid-cols-1 md:grid-cols-${Math.min(prazosArray.length, 3)} gap-4">
            ${prazosArray.map(p => `
              <div class="${p.status.classe} rounded-lg p-4 border-2">
                ...
                <p class="text-xs opacity-75">📅 ${new Date(p.prazo).toLocaleDateString('pt-BR')}</p>
              </div>
            `).join('')}
          </div>
        </div>
  ...
  // Filtrar quadrimestres liberados (se não for admin)
  if (currentUser && currentUser.nivel !== 'ADMIN') {
        ...
        <div class="flex items-center justify-between mb-6 pb-4 border-b-2 border-slate-300">
          ...
          ${prazoQuad ? `
            <div class="text-right">
              ...
              <p class="text-lg font-black text-purple-700">📅 ${new Date(prazoQuad).toLocaleDateString('pt-BR')}</p>
            </div>
          ` : ''}
        </div>
    `;
    
    // Filtrar metas que têm ações neste quadrimestre
...
function abrirModalMonitoramentoQuadrimestre(metaId, quadrimestre) {
  ...
  infoContainer.innerHTML = `
    <div class="flex items-center gap-4">
      ...
      <div class="flex-1">
        ...
        <p class="text-sm text-slate-600">${areasTexto}</p>
        <p class="text-sm font-bold text-purple-700 mt-1">📅 ${quadrimestre}</p>
      </div>
    </div>
  `;
  ...
  // Buscar monitoramento específico do quadrimestre
  ...
  if (acoesVinculadas.length === 0) {
    listaAcoesContainer.innerHTML = '<p class="text-sm text-slate-600 italic">Nenhuma ação vinculada a esta meta</p>';
  } else {
    let htmlAcoes = '';
    acoesVinculadas.forEach((acao, index) => {
      const progressoExistente = acao.progressos?.find(p => p.quadrimestre === quadrimestre);

      // Usar parser BRL robusto e preferir campo numérico salvo
      ...
      htmlAcoes += `
        <div class="bg-white border-2 border-green-300 rounded-lg p-5 shadow-md">
          ...
          <!-- DADOS FIXOS (NÃO EDITÁVEIS) - INFORMAÇÕES DO CADASTRO -->
          <div class="bg-blue-50 rounded-lg p-4 mb-4 border-2 border-blue-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div>
                <label class="block text-xs font-bold text-blue-700 mb-1">Responsável</label>
                <div class="px-3 py-2 bg-white border-2 border-blue-200 rounded-lg text-sm font-semibold text-blue-900">
          ...
          <div class="border-t-2 border-green-200 pt-4 mt-4">
            <h5 class="text-sm font-bold text-green-900 mb-3">📊 Preenchimento do ${quadrimestre}</h5>
            <div class="bg-green-50 rounded-lg p-4 border border-green-200">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                <div>
                  <label class="block text-xs font-bold text-slate-700 mb-1">Resultado Alcançado da Ação (com base no Valor Proposto) *</label>
                  <input type="text" 
              ...
              <div style="margin-bottom:8px;">
                <label class="block text-xs font-bold text-slate-700 mb-1">Execução vs Proposto</label>
                ...
                <div style="font-size:10px;margin-top:6px;color:#374151;font-weight:700;">${!isNaN(valorExecutadoNum) ? formatToBRL(valorExecutadoNum) : (progressoExistente?.valorExecutado || '0')} / ${!isNaN(valorPropostoNum) ? formatToBRL(valorPropostoNum) : (acao.valorProposto || 'N/A')} • ${executadoPct}%</div>
              </div>
              <div>
                ...
                <select 
                  ...
                  <option value="CONCLUIDA" ${progressoExistente?.status === 'CONCLUIDA' ? 'selected' : ''}>✅ Concluída</option>
                  <option value="EM_ANDAMENTO" ${progressoExistente?.status === 'EM_ANDAMENTO' ? 'selected' : ''}>⏳Em Andamento</option>
                  <option value="PREPARATORIA" ${progressoExistente?.status === 'PREPARATORIA' ? 'selected' : ''}>🔧 Preparatória</option>
                  <option value="NAO_INICIADA" ${progressoExistente?.status === 'NAO_INICIADA' ? 'selected' : ''}>⚪ Não Iniciada</option>
                  <option value="PARALISADA" ${progressoExistente?.status === 'PARALISADA' ? 'selected' : ''}>⏸️ Paralisada</option>
                </select>
              </div>
              <div>
                <label class="block text-xs font-bold text-slate-700 mb-1">Análise Crítica *</label>
                <textarea 
                  data-acao-id="${acao.id}" 
                  data-quadrimestre="${quadrimestre}" 
                  data-campo="analise" 
                  ...
                  placeholder="Descreva a análise crítica dos resultados, desafios enfrentados e providências tomadas..."
...
document.getElementById('formMonitoramento').addEventListener('submit', (e) => {
  ...
  if (!resultado || !status || !analise) {
    alert('⚠️ Preencha todos os campos do monitoramento da meta');
    return;
  }
  
  // Inicializar array de monitoramentos se não existir
  if (!meta.monitoramentos) {
    meta.monitoramentos = [];
  }
  
  // Verificar se já existe monitoramento para este quadrimestre
  ...
  // Validar que todas as ações têm os campos obrigatórios preenchidos
  ...
  if (!tudoPreenchido) {
    alert('⚠️ Preencha todos os campos obrigatórios (Resultado Alcançado da Ação, Status e Análise Crítica) de todas as ações');
    return;
  }
  
  // Salvar progressos das ações
  acoesVinculadas.forEach(acao => {
    ...
    if (valorInput && statusSelect && analiseTextarea) {
      ...
      if (valor && statusQuad && analiseQuad) {
        const progressoExistente = acao.progressos.find(p => p.quadrimestre === quadrimestre);
        // Normalizar e salvar também a versão numérica do valor executado
    ...
    // Atualizar status geral da ação baseado no último progresso
  ...
  if (pendentes.length > 0) {
    container.innerHTML = `
      <div class="bg-yellow-100 border-2 border-yellow-500 rounded-xl p-4 flex items-center gap-3 animate-pulse">
        ...
        <div class="flex-1">
          <p class="font-bold text-yellow-900">Atenção: ${pendentes.length} usuário(s) aguardando autorização</p>
          <p class="text-sm text-yellow-800">Acesse a aba "Autorização de Usuários" para aprovar ou rejeitar</p>
        </div>
      </div>
    `;
  } else {
...
function autorizarUsuario(id) {
  ...
  registrarAuditoria('AUTORIZAR_USUARIO', `Usuário ${usuario.nome} foi autorizado`);
  salvarDados();
  renderizarAdmin();
}
...
function rejeitarUsuario(id) {
  ...
  confirmar.innerHTML = `
    <div class="modal-container w-full max-w-md p-6">
      <h3 class="text-xl font-bold text-slate-900 mb-4">Confirmar Rejeição</h3>
      <p class="text-sm text-slate-700 mb-4">Tem certeza que deseja rejeitar o usuário "<strong>${usuario.nome}</strong>"?</p>
      <p class="text-sm text-red-600 mb-4">Esta ação irá remover permanentemente o cadastro.</p>
      <div class="flex gap-3">
        <button onclick="this.closest('.modal-overlay').remove()" class="flex-1 px-4 py-2 border-2 border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 transition font-bold">
          Cancelar
        </button>
...
function excluirUsuarioAdmin(id) {
  ...
  confirmar.innerHTML = `
    <div class="modal-container w-full max-w-md p-6">
      <h3 class="text-xl font-bold text-slate-900 mb-4">Confirmar Exclusão</h3>
      <p class="text-sm text-slate-700 mb-4">Tem certeza que deseja excluir o usuário "<strong>${usuario.nome}</strong>"?</p>
      <div class="flex gap-3">
        <button onclick="this.closest('.modal-overlay').remove()" class="flex-1 px-4 py-2 border-2 border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 transition font-bold">
          Cancelar
        </button>
...
function confirmarExclusaoUsuarioAdmin(id) {
  const usuario = usuarios.find(u => u.id === id);
  if (usuario) {
    registrarAuditoria('EXCLUIR_USUARIO', `Usuário ${usuario.nome} foi removido do sistema`);
    usuarios = usuarios.filter(u => u.id !== id);
    salvarDados();
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="flex-1">
            ...
            <div class="flex gap-3 mt-1 text-xs text-slate-600">
              <span><strong>Usuário:</strong> ${log.usuario}</span>
...
function limparAuditoria() {
  ...
  confirmar.innerHTML = `
    <div class="modal-container w-full max-w-md p-6">
      ...
      <p class="text-sm text-slate-700 mb-4">Tem certeza que deseja limpar todo o histórico de auditoria?</p>
...
document.getElementById('formNovaArea').addEventListener('submit', (e) => {
  ...
  areas.push(nome);
  registrarAuditoria('CRIAR_AREA', `Nova área técnica "${nome}" criada`);
  salvarDados();
...
function editarArea(nomeAntigo) {
  ...
  modal.innerHTML = `
    <div class="modal-container w-full max-w-md p-6">
      ...
      <form id="formEditarArea" class="space-y-4">
        <div>
          <label class="block text-sm font-bold text-slate-700 mb-2">Nome da Área</label>
  ...
  document.getElementById('formEditarArea').addEventListener('submit', (e) => {
    ...
    if (index !== -1) {
      ...
      registrarAuditoria('EDITAR_AREA', `Área técnica "${nomeAntigo}" renomeada para "${novoNome}"`);
      salvarDados();
...
function excluirArea(nome) {
  ...
  confirmar.innerHTML = `
    <div class="modal-container w-full max-w-md p-6">
      <h3 class="text-xl font-bold text-slate-900 mb-4">Confirmar Exclusão</h3>
      <p class="text-sm text-slate-700 mb-4">Tem certeza que deseja excluir a área "<strong>${nome}</strong>"?</p>
      <div class="flex gap-3">
        <button onclick="this.closest('.modal-overlay').remove()" class="flex-1 px-4 py-2 border-2 border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 transition font-bold">
          Cancelar
        </button>
...
function confirmarExclusaoArea(nome) {
  ...
  registrarAuditoria('EXCLUIR_AREA', `Área técnica "${nome}" excluída`);
  salvarDados();
...
document.getElementById('formNovoQuadrimestre').addEventListener('submit', (e) => {
  ...
  if (quadrimestres.includes(nome)) {
    alert('Este quadrimestre já existe');
    return;
  }
...
function confirmarExclusaoQuadrimestre(nome) {
  ...
  registrarAuditoria('EXCLUIR_QUADRIMESTRE', `Quadrimestre "${nome}" excluído`);
  salvarDados();
...
function renderizarMetricas() {
  ...
  if (containerTempo) {
    ...
    if (metas.length === 0) {
      ...
      metas.forEach(meta => {
        ...
        // Calcular tempo médio baseado nos monitoramentos
        let tempoTotal = 0;
        let contador = 0;
        ...
        if (tempoMedio === 0) {
          ...
          statusTempo = `${tempoMedio} dia(s) - Rápido`;
        ...
        htmlTempo += `
          <div class="border-b border-slate-200 pb-3 mb-3 last:border-b-0">
            ...
            <div class="flex items-center gap-2 text-xs text-slate-500">
              <span>📊 ${totalMonitoramentos} preenchimento(s)</span>
              <span>•</span>
              <span>🎯 ${acoesVinculadas.length} ação(ões)</span>
            </div>
          </div>
        `;
      });
  ...
  if (containerGerais) {
    ...
    containerGerais.innerHTML = `
      ...
      <div class="flex items-center justify-between">
        <span class="text-sm text-slate-600">Total de Ações</span>
...
function exportarPDF() {
  try {
    if (typeof html2pdf === 'undefined') {
      alert('❌ Erro: Biblioteca PDF não está disponível. Recarregue a página.');
      return;
    }

    const elemento = document.getElementById('secaoDashboard');
    if (!elemento) {
      alert('⚠️ Por favor, acesse o Painel Geral antes de exportar.');
      return;
    }

    if (!confirm('📄 Exportar Relatório em PDF (A4)?\n\nApenas dados do sistema serão inclusos.')) {
      return;
    }
    ...
    // Dados do módulo de monitoramento (filtrados por perfil)
    ...
    // Calcular Barra de Saúde (apenas se houver dados válidos)
    ...
    // Tendência
    const tendenciaCritico = critico > 15 ? '↑' : critico < 5 ? '↓' : '→';
    const corTendencia = tendenciaCritico === '↑' ? '#dc2626' : tendenciaCritico === '↓' ? '#10b981' : '#6b7280';
    const descricaoTendencia = tendenciaCritico === '↑' ? 'Aumento de ações críticas' : tendenciaCritico === '↓' ? 'Redução de ações críticas' : 'Estável';
        ...
        <div class="page">
          ...
          <div class="header">
            ...
            <div class="header-meta">
              ...
              <strong>Usuário</strong>${usuarioGerador}<br>
          ...
          <div class="executive-summary">
            ...
            ${totalAcoes > 0 
              ? `Este relatório consolida a situação do monitoramento PAS com <strong>${totalAcoes} ações</strong> em acompanhamento. 
                 ${critico > 0 ? `<span class="alert">⚠️ CRÍTICO: Existem ${Math.round(totalAcoes * critico / 100)} ações (${critico}%) que requerem atenção imediata.</span>` : '<span class="success-text">✓ Nenhuma ação em status crítico.</span>'} 
                 Progresso geral: <strong>${progresso}%</strong> com <strong>${concluidas} ações concluídas</strong>.`
              : 'Nenhuma ação foi cadastrada ou monitorada no período. Este relatório apresenta a estrutura padrão de acompanhamento.'
            }
          </div>

          <!-- BARRA DE SAÚDE (APENAS SE HOUVER DADOS) -->
          ${mostrarSaude ? `
          <div class="health-bar-container">
            <div class="health-bar-title">💚 Saúde do Projeto</div>
            <div class="progress-bar">
              <div class="progress-fill" style="width: ${barraLarguraSaude}%;"></div>
            </div>
            <div class="health-stats">
              <div><strong>${barraLarguraSaude}%</strong> conclusão</div>
              <div><strong>${concluidas}/${totalAcoes}</strong> concluídas</div>
            </div>
          </div>
          ` : ''}

          <!-- KPI CARDS - SEMPRE EXIBIDO (4 CARDS DESTACANDO MÉTRICAS) -->
          <div class="kpi-grid">
            <div class="kpi-card critical">
              ...
              <div class="kpi-footer">${Math.round(totalAcoes * critico / 100)} ações</div>
            </div>

            <div class="kpi-card warning">
              <div class="kpi-label">Progresso</div>
              <div class="kpi-value">${progresso}%</div>
              <div class="kpi-footer">avanço geral</div>
            </div>

            <div class="kpi-card info">
              <div class="kpi-label">Em Andamento</div>
              <div class="kpi-value">${pctAndamento}%</div>
              <div class="kpi-footer">${andamento} ações</div>
            </div>

            <div class="kpi-card success">
              <div class="kpi-label">Concluídas</div>
              <div class="kpi-value">${pctConcluidas}%</div>
              <div class="kpi-footer">${concluidas} ações</div>
            </div>
          </div>

          <!-- GRÁFICOS - SEMPRE EXIBIDOS -->
            ...
            <div class="chart-card">
              ...
              <div class="mini-bar-chart">
                <div class="chart-item">
                  <div class="chart-label">Crítico</div>
                ...
                <div class="chart-item">
                  <div class="chart-label">Concluída</div>
                  <div class="chart-bar-container">
          ...
          <!-- TABELA DETALHADA DE STATUS - SEMPRE VISÍVEL -->
          <div class="table-section">
            <div class="section-title">📊 Distribuição Detalhada de Status</div>
            <table>
              <thead>
                <tr>
                  <th>Status</th>
          ...
          <div class="footer">
            <div>
              ...
              <div class="footer-item">Secretaria Estadual de Saúde Pública</div>
              <div class="footer-item"><strong>Usuário:</strong> ${usuarioGerador}</div>
              <div class="footer-item"><strong>Data:</strong> ${dataFormatada}</div>
            </div>
            <div class="footer-right">
              <div class="footer-item"><strong>Horário:</strong> ${horaFormatada}</div>
